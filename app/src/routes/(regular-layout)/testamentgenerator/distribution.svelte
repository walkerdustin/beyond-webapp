<script lang="ts">
	...
	onMount(async () => {
        ...
        let order_ehegatte_member = temp_family_members.filter((member) => order_ehegatte.includes(member.verhältnis));
        let order1_members = temp_family_members.filter((member) => order1.includes(member.verhältnis));
        let order1_5_members = temp_family_members.filter((member) => order1_5.includes(member.verhältnis));
        let order2_members = temp_family_members.filter((member) => order2.includes(member.verhältnis));
        let order2_5_members = temp_family_members.filter((member) => order2_5.includes(member.verhältnis));
    ...
    $:{summe_aktuelle_quote = $family_members.reduce((sum, member) => sum + (member.current_percentage?member.current_percentage:0), 0)};
